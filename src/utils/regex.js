...
const v2LootLogRe =
  /^(?<lootedAt>[^;]*);(?<lootedByAlliance>.*);(?<lootedByGuild>.*);(?<lootedBy>\w+);(?<itemId>[\w@]+);(?<itemName>.*);(?<amount>\d*);(?<lootedFromAlliance>.*);(?<lootedFromGuild>.*);(?<lootedFrom>[\w@]+)$/gm
const aoLootLogRe = /^"(?<lootedAt>[^,]*)","(?<lootedBy>\w+?)","(?<guildName>.*)","(?<item>.*)","(?<itemName>.*) \((?<itemId>[\w@]+)\)","(?<amount>\d*)x .+","(?<lootedFrom>[\w@]+?)"/gm
const albionAnalysisLootLogRe = /^(?<lootedAt>[^;]*);(?<lootedBy>\w+);(?<itemId>[\w@]+);(?<amount>\d*);(?<lootedFrom>[\w@]+)$/gm
